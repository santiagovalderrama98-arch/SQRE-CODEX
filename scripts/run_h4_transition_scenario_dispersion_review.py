#!/usr/bin/env python3
"""Run SQRE H4 transition scenario dispersion review."""
...
from sqre.h4_transition_scenario_dispersion_review.h4_transition_scenario_dispersion_review_pipeline import (
    run_h4_transition_scenario_dispersion_review,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SQRE H4 transition scenario dispersion review")
    parser.add_argument("--input-dir", type=Path, default=Path("data/research/h4_transition_outcome_deep_dive"))
    parser.add_argument("--output-dir", type=Path, default=Path("data/research/h4_transition_scenario_dispersion_review"))
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("data/research/h4_transition_scenario_dispersion_review/h4_transition_scenario_dispersion_review_report.txt"),
    )
...
def main() -> int:
    ...
    try:
        result = run_h4_transition_scenario_dispersion_review(args.input_dir, args.output_dir, args.report, config)
    except Exception as exc:
        print(f"H4 transition scenario dispersion review failed: {exc}", file=sys.stderr)
        return 1
    print("H4 transition scenario dispersion review completed")
    ...
    print(f"Transition family rows: {len(result.family_summaries)}")
    print(f"Source state rows: {len(result.source_state_summaries)}")
    print(f"Target state rows: {len(result.target_state_summaries)}")
    ...
    print(f"Summary path: {result.output_dir / 'h4_transition_scenario_dispersion_review_summary.csv'}")
