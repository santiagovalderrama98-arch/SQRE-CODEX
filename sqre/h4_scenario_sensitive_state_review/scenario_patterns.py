"""Scenario recurrent deviation summaries."""

from __future__ import annotations

from collections import defaultdict

from sqre.h4_scenario_sensitive_state_review.classification import most_common, scenario_recurrent_deviation_class
from sqre.h4_scenario_sensitive_state_review.config import H4ScenarioSensitiveStateReviewConfig
from sqre.h4_scenario_sensitive_state_review.findings import scenario_review_diagnostic
from sqre.h4_scenario_sensitive_state_review.models import ScenarioDeviationDetailRow, ScenarioRecurrentDeviationSummary
from sqre.h4_scenario_sensitive_state_review.state_window_review import mean
...
def build_scenario_recurrent_deviation_summary(
    ...
    for scenario_id, items in sorted(grouped.items()):
        ...
        recurrent = scenario_recurrent_deviation_class(ratio, config)
        summaries.append(
            ScenarioRecurrentDeviationSummary(
                ...
                most_common_primary_deviation_metric=most_common([item.primary_scenario_deviation_metric for item in items], "MIXED"),
