"""CSV and text report writers for H4 transition scenario-sensitive review."""
...
from sqre.h4_transition_scenario_sensitive_review.loader import optional_file_status
...
FAMILY_COLUMNS = [
    "Transition_Family",
    *GROUP_COLUMNS[:-1],
    "Transition_Family_Sensitivity_Diagnostic",
    "Recommended_Follow_Up",
]
SOURCE_COLUMNS = [
    "Source_State",
    *GROUP_COLUMNS[:-1],
    "Source_State_Sensitivity_Diagnostic",
    "Recommended_Follow_Up",
]
TARGET_COLUMNS = [
    "Target_State",
    *GROUP_COLUMNS[:-1],
    "Target_State_Sensitivity_Diagnostic",
    "Recommended_Follow_Up",
]
...
FOCUS_COLUMNS = PROFILE_REVIEW_COLUMNS + ["Focus_Profile_Rationale"]
...
def write_review_outputs(
    ...
    _write_rows(
        result.output_dir / "h4_transition_scenario_sensitive_profile_review.csv",
        result.reviewed_profiles,
        PROFILE_REVIEW_COLUMNS,
    )
    _write_rows(
        result.output_dir / "h4_transition_profile_scenario_deviation_detail.csv",
        result.scenario_details,
        DETAIL_COLUMNS,
    )
    _write_rows(
        result.output_dir / "h4_transition_scenario_recurrent_deviation_summary.csv",
        result.scenario_summaries,
        SCENARIO_COLUMNS,
    )
    _write_group_rows(
        result.output_dir / "h4_transition_family_sensitivity_summary.csv",
    ...
    _write_group_rows(
        result.output_dir / "h4_transition_source_state_sensitivity_summary.csv",
    ...
    _write_group_rows(
        result.output_dir / "h4_transition_target_state_sensitivity_summary.csv",
    ...
    _write_rows(
        result.output_dir / "h4_transition_forward_window_sensitivity_summary.csv",
        result.window_summaries,
        WINDOW_COLUMNS,
    )
    _write_rows(
        result.output_dir / "h4_transition_near_aggregation_candidate_review.csv",
        result.near_candidates,
        NEAR_COLUMNS,
    )
    _write_rows(
        result.output_dir / "h4_transition_focus_profile_review.csv",
        result.focus_profiles,
        FOCUS_COLUMNS,
    )
    _write_rows(
        result.output_dir / "h4_transition_scenario_sensitive_review_summary.csv",
...
def build_report_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    status = optional_file_status(result.dispersion_review_dir, result.transition_deep_dive_dir)
    lines = [
        ...
        f"Transition Deep Dive: {result.transition_deep_dive_dir}",
        ...
        "-----------------------",
        _focus_review_text(result),
        "",
        "Transition Family Sensitivity Review",
        "------------------------------------",
        _group_review_text(result.family_summaries, "family"),
        "",
        "Source State Sensitivity Review",
        "-------------------------------",
        _group_review_text(result.source_state_summaries, "source"),
        "",
        "Target State Sensitivity Review",
        "-------------------------------",
        _group_review_text(result.target_state_summaries, "target"),
        "",
        ...
        "- Expanded H4 historical sample review",
        "",
        "Do Not Change Yet",
        "-----------------",
...
def _write_group_rows(
    ...
    records = [_group_record(row, group_column, diagnostic_column) for row in rows if row is not None]
    pd.DataFrame(records, columns=columns).to_csv(path, index=False)
...
def _group_record(row: object, group_column: str, diagnostic_column: str) -> dict[str, object]:
    raw = asdict(row)
    record = {column: raw.get(column.lower(), "") for column in GROUP_COLUMNS}
    record[group_column] = raw.get("group_value", "")
    record[diagnostic_column] = raw.get("sensitivity_diagnostic", "")
    return record
...
def _summary_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    summary = result.review_summary
    if summary is None:
        return "No H4 transition scenario-sensitive summary was available."
    return "\n".join(
        [
            f"Reviewed profiles: {summary.reviewed_profile_count}",
            f"Focus profiles: {summary.focus_profile_count}",
            ...
            f"H4 transition review readiness: {summary.h4_review_readiness_flag}",
            summary.h4_transition_scenario_sensitive_review_diagnostic,
        ]
    )


def _profile_overview_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    summary = result.review_summary
    if summary is None:
        return "No transition scenario-sensitive profile rows were available."
    return "\n".join(
        [
...
def _focus_review_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    if not result.focus_profiles:
        return "No configured focus transition profiles were available in this review."
    return "\n".join(
        "- "
        f"{row.condition_label} FW={row.forward_window}: sensitivity={row.transition_sensitivity_class}, "
        f"driver={row.primary_deviating_metric}, near={row.near_aggregation_candidate_flag}"
        for row in result.focus_profiles
    )


def _group_review_text(rows: list, label: str) -> str:
    if not rows:
        return f"No {label} sensitivity summaries were available."
    return "\n".join(
        "- "
        f"{row.group_value}: profiles={row.profile_count}, high={row.high_sensitivity_profile_count}, "
        f"moderate={row.moderate_sensitivity_profile_count}, focus={row.focus_profile_count}, "
        f"near={row.near_aggregation_candidate_count}, driver={row.most_common_dispersion_driver}, "
        f"diagnostic={row.sensitivity_diagnostic}"
        for row in rows
    )


def _window_review_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    ...
    return "\n".join(
        ...
        f"moderate={row.moderate_sensitivity_profile_count}, focus={row.focus_profile_count}, "
        f"near={row.near_aggregation_candidate_count}, driver={row.most_common_dispersion_driver}, "
        f"diagnostic={row.window_sensitivity_diagnostic}"
        for row in result.window_summaries
    )


def _scenario_review_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
...
def _driver_review_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
...
def _readiness_text(result: H4TransitionScenarioSensitiveReviewResult) -> str:
    summary = result.review_summary
    if summary is None:
