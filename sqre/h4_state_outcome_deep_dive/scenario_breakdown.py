"""Build scenario-level breakdown for selected H4 state profiles."""

from __future__ import annotations

from sqre.h4_state_outcome_deep_dive.config import H4StateOutcomeDeepDiveConfig
from sqre.h4_state_outcome_deep_dive.findings import scenario_observation_diagnostic
from sqre.h4_state_outcome_deep_dive.models import H4StateProfile, ScenarioBreakdownRow, ScenarioOutcome


def build_scenario_breakdown(
    selected_profiles: list[H4StateProfile],
...
def _matching_outcomes(profile: H4StateProfile, outcomes: list[ScenarioOutcome]) -> list[ScenarioOutcome]:
    return [
        outcome
        for outcome in outcomes
        if outcome.timeframe.strip().upper() == "H4"
        and outcome.condition_type.strip().upper() == "STATE_CONDITION"
