"""Research query execution engine."""
...
def run_research_queries(
    ...
    result_sequence = 1
    for _, query in query_requests.iterrows():
        matches, traces, match_level = find_research_reference_matches(query, reference_store, config)
        trace_rows.extend(traces)
        if matches.empty:
            result_rows.append(_empty_result(result_sequence, query, match_level))
            result_sequence += 1
            continue
        ranked = rank_reference_candidates(matches).head(config.maximum_results_per_query)
        for rank, (_, match) in enumerate(ranked.iterrows(), start=1):
            result_rows.append(_result(result_sequence, query, match, match_level, rank, config))
            result_sequence += 1
    return pd.DataFrame(result_rows, columns=RESULT_COLUMNS), pd.DataFrame(trace_rows, columns=TRACE_COLUMNS)
...
def _result(
    ...
    return {
        **_query_fields(result_sequence, query),
        ...
        "Research_Query_Result_Quality_Class": classify_result_quality(match_level, sample_size, dispersion, config),
        "Research_Query_Evidence_Class": classify_evidence(tier, sample_size, dispersion, config),
        "Result_Rank": rank,
        "Query_Result_Diagnostic": f"Descriptive historical reference matched using {match_level}.",
    }


def _empty_result(result_sequence: int, query: pd.Series, match_level: str) -> dict[str, object]:
    quality = "INPUT_MISSING" if match_level == "INPUT_MISSING" else "NO_USABLE_RESEARCH_QUERY_RESULT"
    evidence = "INPUT_MISSING" if match_level == "INPUT_MISSING" else "INSUFFICIENT_RESEARCH_REFERENCE_EVIDENCE"
    return {
        **_query_fields(result_sequence, query),
        "Matched_Research_Reference_ID": "",
        "Matched_Outcome_Profile_ID": "",
        ...
        "Result_Rank": 0,
        "Query_Result_Diagnostic": "No descriptive historical research reference matched this query.",
    }


def _query_fields(result_sequence: int, query: pd.Series) -> dict[str, object]:
    return {
        "Research_Query_Result_ID": f"RQR_{result_sequence:06d}",
