"""Models for H4 transition scenario dispersion review."""
...
@dataclass(frozen=True)
class TransitionGroupDispersionSummaryRow:
    group_value: str
    profile_count: int
    ...
    dispersion_diagnostic: str
    recommended_follow_up: str


@dataclass(frozen=True)
class ForwardWindowDispersionSummaryRow:
    forward_window: int
    profile_count: int
    research_ready_profile_count: int
    scenario_sensitive_profile_count: int
...
@dataclass(frozen=True)
class H4TransitionScenarioDispersionReviewSummary:
    timeframe: str
    ...
    average_outcome_magnitude_cv: float
    average_direction_alignment_dispersion: float
    h4_transition_dispersion_profile: str
    h4_transition_aggregation_readiness_flag: str
    h4_transition_scenario_dispersion_diagnostic: str
    recommended_follow_up: str


@dataclass(frozen=True)
class H4TransitionScenarioDispersionReviewResult:
    input_dir: Path
    output_dir: Path
    report_path: Path
    ...
    family_summaries: list[TransitionGroupDispersionSummaryRow] = field(default_factory=list)
    source_state_summaries: list[TransitionGroupDispersionSummaryRow] = field(default_factory=list)
    target_state_summaries: list[TransitionGroupDispersionSummaryRow] = field(default_factory=list)
    window_summaries: list[ForwardWindowDispersionSummaryRow] = field(default_factory=list)
    ...
    review_summary: H4TransitionScenarioDispersionReviewSummary | None = None
