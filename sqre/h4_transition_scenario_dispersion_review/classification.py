"""Classification rules for H4 transition scenario dispersion review."""
...
def profile_dispersion_class(forward_range_cv: float, outcome_magnitude_cv: float, config: H4TransitionScenarioDispersionReviewConfig) -> str:
...
def dispersion_driver_class(forward_range_cv: float, outcome_magnitude_cv: float, config: H4TransitionScenarioDispersionReviewConfig) -> str:
...
def transition_profile_readiness_class(
    profile_type: str,
...
def scenario_contribution_class(high_deviation_profile_ratio: float, config: H4TransitionScenarioDispersionReviewConfig) -> str:
...
def h4_aggregation_readiness_flag(aggregation_candidates: int, scenario_sensitive: int, research_ready: int) -> str:
    if aggregation_candidates > 0 and scenario_sensitive < research_ready:
        return "READY_FOR_SELECTIVE_TRANSITION_AGGREGATION_REVIEW"
    return "REQUIRES_TRANSITION_SCENARIO_DISPERSION_REVIEW"
