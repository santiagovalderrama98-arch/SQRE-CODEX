"""Load H4 transition deep dive outputs for scenario dispersion review."""
...
PROFILE_INVENTORY_FILE = "h4_transition_deep_dive_profile_inventory.csv"
SCENARIO_BREAKDOWN_FILE = "h4_transition_scenario_breakdown.csv"
OUTCOME_STATISTICS_FILE = "h4_transition_outcome_statistics.csv"
SCENARIO_COMPARISON_FILE = "h4_transition_scenario_comparison_matrix.csv"
FAMILY_SUMMARY_FILE = "h4_transition_family_summary.csv"
DEEP_DIVE_SUMMARY_FILE = "h4_transition_deep_dive_summary.csv"
...
def load_outcome_statistics(input_dir: Path | str) -> list[OutcomeStatisticsInput]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"H4 transition outcome statistics file not found: {path}")
    frame = pd.read_csv(path)
...
def optional_file_status(input_dir: Path | str) -> dict[str, bool]:
    ...
    return {
        ...
        FAMILY_SUMMARY_FILE: (root / FAMILY_SUMMARY_FILE).exists(),
...
def _profile_row(row: pd.Series) -> ProfileInventoryRow:
    return ProfileInventoryRow(
        ...
        transition_research_class=text_value(row, "Transition_Research_Class"),
