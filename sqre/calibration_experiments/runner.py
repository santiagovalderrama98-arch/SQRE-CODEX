"""Subprocess runner for SQRE calibration experiments."""
...
from sqre.calibration_experiments.models import ExperimentRun, ExperimentRunResult
...
class CalibrationExperimentRunner:
    """Run one calibration experiment through the existing SQRE CLIs."""

    def __init__(self, pip_size: float = 0.0001) -> None:
        self.pip_size = pip_size

    def run(self, experiment_run: ExperimentRun, *, skip_existing: bool = False) -> ExperimentRunResult:
        started_at = _now()
        _create_output_dirs(experiment_run)
        if not experiment_run.ohlc_path.exists():
            return _result(experiment_run, "MISSING_INPUT", f"OHLC file not found: {experiment_run.ohlc_path}", started_at)
...
def _commands(experiment_run: ExperimentRun, *, pip_size: float) -> list[tuple[str, list[str]]]:
...
def _create_output_dirs(experiment_run: ExperimentRun) -> None:
...
def _final_report_path(experiment_run: ExperimentRun) -> Path:
...
def _result(experiment_run: ExperimentRun, status: str, message: str, started_at: str) -> ExperimentRunResult:
