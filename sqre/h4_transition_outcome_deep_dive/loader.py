"""Load H4 transition outcome deep dive inputs."""
...
PRICE_OUTCOME_CANDIDATES = [
    Path("research/price_outcomes.csv"),
    Path("price_outcomes.csv"),
    Path("data/research/price_outcomes.csv"),
]
STATE_TRANSITION_CANDIDATES = [
    Path("processed/state_transitions.csv"),
    Path("state_transitions.csv"),
    Path("data/processed/state_transitions.csv"),
]
H4_SCENARIOS = [
    "eurusd_h4_period_1",
    "eurusd_h4_period_2",
...
def load_price_outcome_profiles(h4_d1_research_dir: Path | str) -> list[H4TransitionProfileInput]:
...
def find_price_outcomes_file(scenario_dir: Path | str) -> Path | None:
    return _find_candidate(scenario_dir, PRICE_OUTCOME_CANDIDATES)


def find_state_transitions_file(scenario_dir: Path | str) -> Path | None:
    return _find_candidate(scenario_dir, STATE_TRANSITION_CANDIDATES)
...
def optional_input_file_status(h4_d1_research_dir: Path | str, validation_output_dir: Path | str) -> dict[str, bool]:
    ...
    status = {
        "h4_d1_transition_research_profiles": (research_root / "h4_d1_transition_research_profiles.csv").exists(),
    ...
    for scenario_id in H4_SCENARIOS:
        scenario_dir = validation_root / scenario_id
        status[f"{scenario_id}_condition_price_outcome_summary"] = find_scenario_outcome_file(scenario_dir) is not None
        status[f"{scenario_id}_price_outcomes"] = find_price_outcomes_file(scenario_dir) is not None
        status[f"{scenario_id}_state_transitions"] = find_state_transitions_file(scenario_dir) is not None
    return status
...
def _profile_from_row(row: pd.Series) -> H4TransitionProfileInput:
    return H4TransitionProfileInput(
...
def _canonicalize_columns(frame: pd.DataFrame, required_columns: list[str], source_path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Missing required H4 transition deep dive column(s) in {source_path}: {', '.join(missing)}")
...
def _find_candidate(scenario_dir: Path | str, candidates: list[Path]) -> Path | None:
    root = Path(scenario_dir)
