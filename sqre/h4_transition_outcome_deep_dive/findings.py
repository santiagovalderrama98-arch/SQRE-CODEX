"""Descriptive findings for H4 transition outcome deep dive."""
...
def stability_class(forward_range_cv: float, outcome_magnitude_cv: float, config: H4TransitionOutcomeDeepDiveConfig) -> str:
...
def scenario_observation_diagnostic(profile_type: str, sample_size: int, config: H4TransitionOutcomeDeepDiveConfig) -> str:
    ...
    if profile_type == "SCENARIO_SENSITIVE_OBSERVATION":
        return "Scenario observation belongs to a scenario-sensitive H4 transition profile"
    if profile_type == "SAMPLE_CONSTRAINED_OBSERVATION":
        return "Scenario observation belongs to a sample-constrained H4 transition profile"
    return "Scenario observation supports descriptive H4 transition outcome review"
...
def outcome_profile_diagnostic(profile_type: str, stability: str) -> str:
    ...
    return "H4 transition outcome profile requires further descriptive review"
...
def outcome_profile_follow_up(stability: str, profile_type: str) -> str:
    ...
    if profile_type == "SCENARIO_SENSITIVE_OBSERVATION":
        return "Review scenario-sensitive H4 transition profile separately."
    if stability == "HIGH_DISPERSION":
...
def scenario_deviation_class(mean_normalized_deviation: float, config: H4TransitionOutcomeDeepDiveConfig) -> str:
...
def transition_summary_diagnostic(ready_count: int, sample_count: int, scenario_count: int, high_count: int) -> str:
    if high_count:
        return "H4 transition label includes profiles requiring dispersion review"
    if scenario_count:
        return "H4 transition label includes scenario-sensitive observation profiles"
    if sample_count:
        return "H4 transition label includes sample-constrained observation profiles"
    if ready_count:
        return "H4 transition label contains research-ready descriptive profiles"
    return "H4 transition label requires further descriptive review"


def transition_summary_follow_up(sample_count: int, scenario_count: int, high_count: int) -> str:
    if high_count:
