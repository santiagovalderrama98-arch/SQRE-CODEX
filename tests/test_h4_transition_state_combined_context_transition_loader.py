...
def test_transition_loader_maps_profile_dispersion_diagnostics(tmp_path: Path):
    transition_dir = tmp_path / "transition"
    dispersion_dir = tmp_path / "transition_dispersion"
    transition_dir.mkdir()
    dispersion_dir.mkdir()
    (transition_dir / "h4_transition_deep_dive_profile_inventory.csv").write_text(
        "Transition_Label,Forward_Window_Candles,Sample_Size\n"
        "EXPANSION -> CONSOLIDATION,12,42\n",
        encoding="utf-8",
    )
    (dispersion_dir / "h4_transition_profile_dispersion_diagnostics.csv").write_text(
        "Transition_Label,Forward_Window,Profile_Dispersion_Class,Transition_Profile_Readiness_Class\n"
        "EXPANSION -> CONSOLIDATION,12,HIGH_DISPERSION,RESEARCH_READY\n",
        encoding="utf-8",
    )
    config = H4TransitionStateCombinedContextReviewConfig(
        h4_state_deep_dive_dir=tmp_path / "empty_state_deep",
        ...
        h4_state_sensitive_dir=tmp_path / "empty_state_sensitive",
        h4_transition_deep_dive_dir=transition_dir,
        h4_transition_dispersion_dir=dispersion_dir,
        h4_transition_sensitive_dir=tmp_path / "empty_transition_sensitive",
        partial_complement_dir=tmp_path / "empty_partial_complement",
        partial_validation_dir=tmp_path / "empty_partial_validation",
    )
    ...
    assert len(contexts) == 1
    assert contexts[0].dispersion_status == "HIGH_DISPERSION"
