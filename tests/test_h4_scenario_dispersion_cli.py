...
def test_cli_runs_with_synthetic_data(tmp_path: Path):
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_h4_scenario_dispersion_review.py",
            "--input-dir",
            str(input_dir),
            "--output-dir",
            str(output_dir),
    ...
    assert "H4 scenario dispersion review completed" in result.stdout
    assert "Profile dispersion diagnostics: 4" in result.stdout
    assert (output_dir / "h4_scenario_dispersion_review_summary.csv").exists()
