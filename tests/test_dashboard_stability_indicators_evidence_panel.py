...
def test_evidence_stability_panel_is_generated_from_dashboard_inputs(tmp_path):
    ...
    panel = build_evidence_stability_panel(cards, frames["evidence_panel"])

    assert len(panel) == 1
    assert panel.iloc[0]["Snapshot_Result_Count"] == 3
