...
def test_cli_runs_with_synthetic_data(tmp_path: Path):
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_h4_transition_outcome_deep_dive.py",
            "--h4-d1-research-dir",
            str(research_dir),
            ...
            "--report",
            str(report_path),
            "--include-sample-constrained-observations",
            "true",
        ],
        cwd=repo_root,
        check=False,
    ...
    assert "H4 transition outcome deep dive completed" in result.stdout
    assert "Selected profiles: 3" in result.stdout
    assert (output_dir / "h4_transition_deep_dive_summary.csv").exists()
